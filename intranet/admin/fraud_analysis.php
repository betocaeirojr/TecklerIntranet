...
$dimensions = array('date','pagePath', 'city', 'networkLocation', 'country','landingPagePath', 'pageDepth');
...
<form method="post" action="fraud_analysis.php">
	<table border=1>
		...
		<tr> <!--  Filter by Referrer -->
			...
			<td><select name="referrer_select" id="referrer_select" onchange=ActiveFilterRPS()>
					...
					<option value = "fullReferrer =~ (tumblr.com(\/(r(\D){1,})))$" selected>Tumblr Redirect</option>>
...
if (isset($_POST['SubmitSpecific']) or isset($_POST['SubmitStarting']) ) {
	...
	foreach($ga->getResults() as $result){
		echo "<tr>\n";
		echo "<td>" . date('Y-m-d H:i:s', strtotime($result->getDate())) . "</td>\n";
