...
$filter = 'pagePath == /pt/user/new_user && previousPagePath =~ ^\/pt(\/[^\/]+){2,}$ && previousPagePath =~ (-)[0-9]\d{1,}$';
...
<h2> Portuguese Home Leading to New User in Portuguese</h2>
<table border=1>
	<tr>
		<thead>
			<th>Date</th>
...
<?php
// Filtering New User Other from HOME
$filter = 'pagePath == /pt/user/new_user && previousPagePath == /pt/home';
$ga = new gapi($gaUsername, $gaPassword);
$mostPopular = $ga->requestReportData($profileId, $dimensions, $metrics, $sort, ($filter!='null'? $filter : null ), $fromDate, $toDate, 1, 10000);
...
<?php echo "<BR><B>Total Number of Returned Itens is: </B> $ReturnedCount_2 !"; ?>
