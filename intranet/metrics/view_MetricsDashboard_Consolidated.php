...
//$contactInvitationSents_RawInformation				= $UserMetrics->getContactInvitationSent();
$contentCreators_TotalVisitors_Ratio_RawInformation	= $UserMetrics->getContentCreators_TotalVisitorsRatio();

$trafficSource_RawInformation						= $AudienceMetrics->getTrafficSourceInfoPerDay();
$visitorsPerTrafficSource_RawInformation 			= $trafficSource_RawInformation['Visitors'];
$visitsPerTrafficSource_RawInformation 				= $trafficSource_RawInformation['Visits'];
$pageviewsPerTrafficSource_RawInformation 			= $trafficSource_RawInformation['Pageviews'];
...
<!-- ------------------------------------------------------------- -->
		<p>Content Creators Per Day</p>
		<table border=1>
			<tr>
				<?php
					$keys = array_keys($contentCreators_TotalVisitors_Ratio_RawInformation[0]);
					foreach ($keys as $key => $value) {
						echo "<th align=center>";
						echo $value;
						echo "</th>";
					}
					reset($contentCreators_TotalVisitors_Ratio_RawInformation);
				?>
			</tr>
			<?php 
				arsort($contentCreators_TotalVisitors_Ratio_RawInformation);
				// We'll show only the first 90 results...
				$counter = 0 ;
				foreach ($contentCreators_TotalVisitors_Ratio_RawInformation as $ekey => $evalue) {
					if ($counter < 90 ){
						echo "<tr>";		
						foreach ($evalue as $ikey => $ivalue) {
							echo "<TD align=center>";
...
<!-- ------------------------------------------------------------- -->
		...
		<!-- ------------------------------------------------------------- -->
		<p>Visitors Per Traffic Source </p>
		<table border=1>
			<tr>
				<?php
					$keys = array_keys($visitorsPerTrafficSource_RawInformation[0]);
					foreach ($keys as $key => $value) {
						echo "<th align=center>";
						echo $value;
						echo "</th>";
					}
					reset($visitorsPerTrafficSource_RawInformation);
				?>
			</tr>
			<?php 
				//arsort($visitorsPerTrafficSource_RawInformation);
				foreach ($visitorsPerTrafficSource_RawInformation as $ekey => $evalue) {
					echo "<tr>";		
					foreach ($evalue as $ikey => $ivalue) {
						echo "<TD align=center>";
						if ($ikey == 'Date') {
		...
		<!-- ------------------------------------------------------------- -->
		<p>Visits Per Traffic Source </p>
		<table border=1>
			<tr>
				<?php
					$keys = array_keys($visitsPerTrafficSource_RawInformation[0]);
					foreach ($keys as $key => $value) {
						echo "<th align=center>";
						echo $value;
						echo "</th>";
					}
					reset($visitsPerTrafficSource_RawInformation);
				?>
			</tr>
			<?php 
				//arsort($visitsPerTrafficSource_RawInformation);
				foreach ($visitsPerTrafficSource_RawInformation as $ekey => $evalue) {
					echo "<tr>";		
					foreach ($evalue as $ikey => $ivalue) {
						echo "<TD align=center>";
						if ($ikey == 'Date') {
		...
		<!-- ------------------------------------------------------------- -->
		<p>Pageviews Per Traffic Source </p>
		<table border=1>
			<tr>
				<?php
					$keys = array_keys($pageviewsPerTrafficSource_RawInformation[0]);
					foreach ($keys as $key => $value) {
						echo "<th align=center>";
						echo $value;
						echo "</th>";
					}
					reset($pageviewsPerTrafficSource_RawInformation);
				?>
			</tr>
			<?php 
				//arsort($pageviewsPerTrafficSource_RawInformation);
				foreach ($pageviewsPerTrafficSource_RawInformation as $ekey => $evalue) {
					echo "<tr>";		
					foreach ($evalue as $ikey => $ivalue) {
						echo "<TD align=center>";
						if ($ikey == 'Date') {
		...
		<!-- ------------------------------------------------------------- -->
		<p>Invitations Sent by Email </p>
		<table border=1>
			<tr>
				<?php
					$keys = array_keys($contactInvitationSents_RawInformation[0]);
					foreach ($keys as $key => $value) {
						echo "<th align=center>";
						echo $value;
						echo "</th>";
					}
					reset($contactInvitationSents_RawInformation);
				?>
			</tr>
			<?php 
				//arsort($pageviewsPerTrafficSource_RawInformation);
				foreach ($contactInvitationSents_RawInformation as $ekey => $evalue) {
					echo "<tr>";		
					foreach ($evalue as $ikey => $ivalue) {
						echo "<TD align=center>";
						if ($ikey == 'Date') {
